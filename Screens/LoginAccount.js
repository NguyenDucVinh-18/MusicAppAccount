...
import AsyncStorage from '@react-native-async-storage/async-storage'; // Để lưu trữ thông tin người dùng
...
export default function LoginAccount({ navigation }) {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      // Log token (nếu có) hoặc thông tin người dùng
      console.log('Login successful, token:', response.data.token);

      // Lưu thông tin người dùng vào AsyncStorage hoặc Context nếu cần thiết
      await AsyncStorage.setItem('userToken', response.data.token);  // Giả sử response trả về token

      // Sau khi lưu thông tin, chuyển hướng tới màn hình Home
